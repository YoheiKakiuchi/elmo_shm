...
#include "wheel_shm.h"     // elmo_shm/include
  ...
  while( !(*stop_flag) ) {
    ...
    for(int n = 0; n < ROW_NUM; n++) {
      ...
      for(int i = 0; i < COLUMN_NUM; i++) {
        ...
        { // servo state
          unsigned short stat = ((unsigned int)(shm->hole_status[idx]) & 0xFFFF0000) >> 16;
        ...
        { // current
          double cur_rate = shm->motor_rate[idx];
          ...
          fprintf(print_strm, "%10.3f ", shm->motor_current[idx]);
          char_color(print_strm, 0);
          next_row(print_strm);
        }
        { // temp
          double temp = shm->motor_temp[idx];
          if (temp > 110.0) {
            char_color(print_strm, 45); // purple
          } else if (temp > 100.0) {
            char_color(print_strm, 41); // red
        ...
        { // extra
          unsigned short stat_extra = (shm->hole_status[idx] & 0x0000FFFF);
          unsigned   int stat_elmo  = shm->servo_state[idx];
          { // elmo
            unsigned int col = 0;
