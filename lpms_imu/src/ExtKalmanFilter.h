...
//
// copy from hrpsys-base/rtc/KalmanFilter/EKFilter.h
//
...
#endif
  void main_one (Eigen::Quaternion<double>& q, const hrp::Vector3& acc, const hrp::Vector3& gyro)
  {
    Eigen::Matrix3d fuzzyR;
    calcRWithFuzzyRule(fuzzyR, acc, gyro);
    prediction(gyro);
    correction(acc, fuzzyR);
    /* ekf_filter.printAll(); */
    Eigen::Quaternion<double> xq(x[0], x[1], x[2], x[3]);
    //rpy = hrp::rpyFromRot(q.toRotationMatrix());
    q = xq.normalized();
  };
